...
export const NeoReport = ({
  ...
  const populateReport = (debounced = true) => {
    ...
    var numericFields =
      REPORT_TYPES[type].selection && fields
        ? Object.keys(REPORT_TYPES[type].selection).filter(
            (field) =>
              REPORT_TYPES[type].selection[field].type ==
                SELECTION_TYPES.NUMBER &&
              !REPORT_TYPES[type].selection[field].multiple
          )
        : [];
    var numericOrDatetimeFields =
      REPORT_TYPES[type].selection && fields
        ? Object.keys(REPORT_TYPES[type].selection).filter(
            (field) =>
              REPORT_TYPES[type].selection[field].type ==
                SELECTION_TYPES.NUMBER_OR_DATETIME &&
              !REPORT_TYPES[type].selection[field].multiple
          )
        : [];
    var textFields =
    ...
    var optionalFields =
      REPORT_TYPES[type].selection && fields
        ? Object.keys(REPORT_TYPES[type].selection).filter(
            (field) => REPORT_TYPES[type].selection[field].optional == true
          )
        : [];
    ...
    if (REPORT_TYPES[type].selection) {
      Object.keys(REPORT_TYPES[type].selection).forEach((field, i) => {
        if (REPORT_TYPES[type].selection[field].multiple && selection[field]) {
          selection[field].forEach((f, i) =>
            numericFields.push(field + "(" + f + ")")
          );
        }
      });
    }
    ...
    const defaultKeyField = REPORT_TYPES[type].selection
      ? Object.keys(REPORT_TYPES[type].selection).find(
          (field) => REPORT_TYPES[type].selection[field].key == true
        )
      : undefined;
    const useRecordMapper = REPORT_TYPES[type].useRecordMapper == true;
    const useNodePropsAsFields =
      REPORT_TYPES[type].useNodePropsAsFields == true;
    if (debounced) {
  ...
  if (disabled) {
    ...
    return (
      <div
        style={{
          height: "100%",
          marginTop: "0px",
          overflow: REPORT_TYPES[type].allowScrolling ? "auto" : "hidden",
        }}
      >
        <ChartType
          records={records}
          ...
          <div style={{ display: "flex" }}>
            <Tooltip
              title={
                "Over " +
                rowLimit +
                " row(s) were returned, results have been truncated."
              }
              placement="left"
              aria-label="host"
            >
    ...
    return (
      <NeoCodeViewerComponent
        value={
          "Query was aborted - it took longer than " +
          queryTimeLimit +
          "s to run. \n" +
